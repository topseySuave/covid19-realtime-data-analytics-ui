...
export default function LeftPanel({ panelData }) {

	const getProjectedData = (panelData) => {
		if (panelData.oneWeekProjection){
			let nextFortnight: Array<string> = []
			for (let i =0; i < 15; i++){
				let n = (panelData.oneWeekProjection * (0.9+(Math.random()*0.2))).toFixed()
	...
	return (
			<section className="w-1/4 h-full bg-gray-900 z-10 absolute left-0 top-0 text-white overflow-y-scroll" id="scrollbar">
					...
					<div className="absolute bottom-0 ml-5">
						...
						{panelData.affectedCountries ?
							<>
								...
								<h1 className="text-2xl text-gray-400 text-white">Affected Countries</h1>
							</>
							:
				...
				<div className="mt-10 px-2">
					...
					<Card type="graph" title="Cases Rate" graphData={data} />
					{
						panelData.dropRate &&
